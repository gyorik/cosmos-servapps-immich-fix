...
for (const file of servapps) {
  ...
  for (const screenshot of screenshots) {
    servapp.screenshots.push(`https://gyorik.github.io/cosmos-servapps-official/servapps/${file}/screenshots/${screenshot}`)
  }

  if(fs.existsSync(`./servapps/${file}/artefacts`)) {
    const artefacts = fs.readdirSync(`./servapps/${file}/artefacts`)
    for(const artefact of artefacts) {
      servapp.artefacts[artefact] = (`https://gyorik.github.io/cosmos-servapps-official/servapps/${file}/artefacts/${artefact}`)
    }
  }

  servapp.icon = `https://gyorik.github.io/cosmos-servapps-official/servapps/${file}/icon.png`
  servapp.compose = `https://gyorik.github.io/cosmos-servapps-official/servapps/${file}/cosmos-compose.json`
...
for (const servapp of servappsJSON) {
  ...
  for (let i = 0; i < servapp.screenshots.length; i++) {
    servapp.screenshots[i] = servapp.screenshots[i].replace('https://gyorik.github.io/cosmos-servapps-official', 'http://localhost:3000')
  }
  for (const artefact in servapp.artefacts) {
    servapp.artefacts[artefact] = servapp.artefacts[artefact].replace('https://gyorik.github.io/cosmos-servapps-official', 'http://localhost:3000')
  }
}
